...
const api = {
  ...
  GetSystemRoleList(params) {
    return request.get('/Permission/GetSystemRoleList', {
      params: params
    })
  },
  // 新增角色。限当前机构。liuyw
...
export default class role {
  ...
  GetSystemRoleList() {
    const that = this;
    this.api.GetSystemRoleList().then(function (res) {
      if (res.isCompleted) {
        that.RoleList = res.data.concat(that.RoleList);
        that.userForm.roleId = res.data[0].roleID;
      }
      else {
        Vue.prototype.$message({
